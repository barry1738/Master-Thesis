...
mu_ex = np.zeros((d, 1))
std_ex = 1
Xh_0 = np.random.multivariate_normal(mu_ex.flatten(), std_ex**2 * np.eye(d), N).T
...
for i in range(M - 1, 0, -1):
    ...
    ) + sigma_tilde[i] * np.random.randn(d, N)


fig, ax = plt.subplots()
ax.imshow(Xh_0.reshape(28, 28).T, cmap="gray")
ax.axis("off")
plt.show()

# Compute mean and std from discrete data
mu_sde = np.sum(Xh_0, axis=1) / N
cov_sde = np.cov(Xh_0.T) * (1 - 1 / N)
