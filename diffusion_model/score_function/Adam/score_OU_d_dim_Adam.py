...
class Build_Data(Dataset):
    # Constructor
    def __init__(self, dim, m, T, mu_0, sigma_0, beta):
        self.dim = dim
        self.m = m
        self.T = T
        self.mu_0 = mu_0
    ...
    def create_data(self):
        ...
        X0 = np.random.normal(self.mu_0, self.sigma_0, (self.m, self.dim))
        t = np.sort(np.append(self.T * np.random.rand(self.m - 1), self.T)).reshape(-1, 1)
        noise = np.random.randn(self.m, self.dim)
...
def main():
    ...
    data_set = Build_Data(dim, m, T, mu_0, sigma_0, beta)
    ...
    # ... run SDE to generate Xt ...
    data_set_test = Build_Data(dim, m_test, T, mu_0, sigma_0, beta)
    data_set_test.create_data()
    x_test = data_set_test.xin
    y_test = model(torch.tensor(x_test, device=device)).cpu().detach().numpy()
    s_test = data_set_test.score_exact(
        x_test[:, 0:-1].reshape(-1, dim), x_test[:, -1].reshape(-1, 1)
    )

    # Compute the 2-norm error
    # ||A||_2 = ( sum_i,j( abs(a_ij)^2 ) ) ^ (1/2)
    error_2norm = np.linalg.norm(y_test - s_test)
