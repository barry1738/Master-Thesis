...
from model_func import predict_dx, predict_dy, predict_dxx, predict_dyy


def projection_step(model, points, rhs_vec, device):
    ...
    def compute_loss_res(model, params, x, y, Rf_proj):
        """Compute the residual loss function."""
        pred = predict_dxx(model, params, x, y) + predict_dyy(model, params, x, y)
        loss_res = pred - Rf_proj
        return loss_res

    def compute_loss_bd(model, params, x, y, nx, ny, Rf_bd):
        """Compute the boundary loss function."""
        pred = (
            predict_dx(model, params, x, y) * nx + predict_dy(model, params, x, y) * ny
        )
        loss_bd = pred - Rf_bd
        return loss_bd
    ...
    for iter in range(Niter):
        ...
        jac_res_dict = vmap(
            ...
            out_dims=0,
        )(model, params, x_inner, y_inner, Rf_proj)

        jac_bd_dict = vmap(
            jacrev(compute_loss_bd, argnums=1),
            in_dims=(None, None, 0, 0, 0, 0, 0),
            out_dims=0,
        )(model, params, x_bd, y_bd, nx, ny, Rf_bd)
        ...
        l_vec_res = compute_loss_res(model, params, x_inner, y_inner, Rf_proj)
        l_vec_bd = compute_loss_bd(model, params, x_bd, y_bd, nx, ny, Rf_bd)
        l_vec_res_valid = compute_loss_res(model, params, x_inner_v, y_inner_v, Rf_proj_v)
        l_vec_bd_valid = compute_loss_bd(model, params, x_bd_v, y_bd_v, nx_v, ny_v, Rf_bd_v)
        ...
        if iter % 1000 == 0:
            ...
            dloss_bd_dp = grad(
                lambda primal: torch.sum(
                    compute_loss_bd(model, primal, x_bd, y_bd, nx, ny, Rf_bd) ** 2
                ), argnums=0,
            )(params)
