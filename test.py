...
class Model(nn.Module):
    ...
    def forward_dxx(self, model, params, x, y):
        ...
        output, vjpfunc = vjp(
            lambda primal: self.forward_dx(model, params, primal, y), x
        )
        return vjpfunc(torch.ones_like(output))[0]
    ...
    def forward_dyy(self, model, params, x, y):
        ...
        output, vjpfunc = vjp(
            lambda primal: self.forward_dy(model, params, x, primal), y
        )
        return vjpfunc(torch.ones_like(output))[0]
...
def compute_loss_Res(model, params, x, y, Rf_inner):
    ...
    laplace = model.forward_dxx(model, params, x, y) + model.forward_dyy(
        model, params, x, y
    )
...
def main():
    ...
    X_inner, Y_inner = (
        X_inner_torch[:, 0].reshape(-1, 1),
        X_inner_torch[:, 1].reshape(-1, 1),
    )
    X_bd, Y_bd = X_bd_torch[:, 0].reshape(-1, 1), X_bd_torch[:, 1].reshape(-1, 1)
    X_inner_valid, Y_inner_valid = (
        X_inner_valid_torch[:, 0].reshape(-1, 1),
        X_inner_valid_torch[:, 1].reshape(-1, 1),
    )
    X_bd_valid, Y_bd_valid = (
        X_bd_valid_torch[:, 0].reshape(-1, 1),
        X_bd_valid_torch[:, 1].reshape(-1, 1),
    )

    model = Model(2, [20, 20], 2).to(device)  # hidden layers = [...](list)
    print(model)
    ...
    a = functional_call(model, u_params, (X_inner, Y_inner))
    b = model.forward_dx(model, u_params, X_inner, Y_inner)
    print(f"a = {a.size()}")
    print(f"b = {b.size()}")
    ...
    for step in range(Niter):
        ...
        Jac_res = torch.hstack(
            [v.view(X_inner.size(0), -1) for v in jac_res_dict.values()]
        )
        ...
        L_vec_res_valid = compute_loss_Res(
            model, u_params, X_inner_valid, Y_inner_valid, Rf_inner_valid
        )
        L_vec_b_valid = compute_loss_Bd(
            model, u_params, X_bd_valid, Y_bd_valid, Rf_bd_valid
        )
        ...
        L_vec_res_valid = L_vec_res_valid / torch.sqrt(
            torch.tensor(X_inner_valid.size(0))
        )
        ...
        loss = torch.sum(L_vec_res**2) + torch.sum(L_vec_b**2)
        loss_valid = torch.sum(L_vec_res_valid**2) + torch.sum(L_vec_b_valid**2)
        ...
        if step % 50 == 0:
            ...
            dloss_res_dparams_flatten = nn.utils.parameters_to_vector(
                dloss_res_dparams.values()
            ...
            dloss_bd_dparams_flatten = nn.utils.parameters_to_vector(
                dloss_bd_dparams.values()
