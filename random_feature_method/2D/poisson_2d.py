    ...
    def assemble_matrix(self, weights, biases, points, J_n, Q):
        ...
        for i in range(self.M_p):
            for j in range(self.M_p):
                ...
                A_predict_dxx = torch.vmap(
                    self.predict_dxx, in_dims=(2, 1, None, None, None, None), out_dims=1)(
                        weights[:, :, :, i], biases[:, :, i],
                        internal_points[i][j][0], internal_points[i][j][1], 
                        i, j
                    ).squeeze()
                
                A_predict_dyy = torch.vmap(
                ...
                if i == 0:  # left boundary
                    A_B_1[j * Q : (j + 1) * Q, J_n_begin : J_n_begin + J_n] = (
                        torch.vmap(
                            self.predict,
                            in_dims=(2, 1, None, None, None, None),
                            out_dims=1,
                        )(
                            weights[:, :, :, i],
                            biases[:, :, i],
                            boundary_points_left[i][j][0],
                            boundary_points_left[i][j][1],
                            i,
                            j,
                        ).squeeze()
                    )
                ...
                if i == self.M_p - 1:  # right boundary
                    A_B_2[j * Q : (j + 1) * Q, J_n_begin : J_n_begin + J_n] = (
                        torch.vmap(
                            ...
                            weights[:, :, :, i],
                            biases[:, :, i],
                            boundary_points_right[i][j][0],
                            boundary_points_right[i][j][1],
                            i,
                            j,
                        ).squeeze()
                    )
                ...
                if j == 0:  # bottom boundary
                    A_B_3[i * Q : (i + 1) * Q, J_n_begin : J_n_begin + J_n] = (
                        torch.vmap(
                            ...
                            weights[:, :, :, i],
                            biases[:, :, i],
                            boundary_points_bottom[i][j][0],
                            boundary_points_bottom[i][j][1],
                            i,
                            j,
                        ).squeeze()
                    )
                ...
                if j == self.M_p - 1:  # top boundary
                    A_B_4[i * Q : (i + 1) * Q, J_n_begin : J_n_begin + J_n] = (
                        torch.vmap(
                            ...
                            weights[:, :, :, i],
                            biases[:, :, i],
                            boundary_points_top[i][j][0],
                            boundary_points_top[i][j][1],
                            i,
                            j,
                        ).squeeze()
                    )
