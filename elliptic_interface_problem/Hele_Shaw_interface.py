...
def main():
    ...
    x_inner_o_v, y_inner_o_v = mesh.interface_points(1000)
    x_inner_i_v, y_inner_i_v = mesh.interface_points(1000)
    x_inner_v = torch.vstack((x_inner_o_v, x_inner_i_v))
    y_inner_v = torch.vstack((y_inner_o_v, y_inner_i_v))
    x_bd_v, y_bd_v = mesh.interface_points(1000)
    x_if_v, y_if_v = mesh.interface_points(1000)
    z_inner_v = torch.vstack((torch.ones_like(x_inner_o_v), -torch.ones_like(x_inner_i_v)))
    ...
    for key, val in u_params.items():
        if "ln_in.weight" in key:
            nn.init.xavier_uniform_(val, gain=10.0)
    ...
    Rf_bd_v = torch.zeros_like(x_if_v)
    ...
    while overfitting:
        ...
        eps = 1e-3
        theta_o = torch.tensor(2 * np.pi * qmc.LatinHypercube(d=1).random(n=500))
        theta_i = torch.tensor(2 * np.pi * qmc.LatinHypercube(d=1).random(n=500))
        radius_o = mesh.func(theta_o) + eps
        radius_i = mesh.func(theta_i) - eps
        x_inner = torch.vstack((radius_o*torch.cos(theta_o), radius_i*torch.cos(theta_i)))
        y_inner = torch.vstack((radius_o*torch.sin(theta_o), radius_i*torch.sin(theta_i)))

        theta_bd = torch.tensor(2 * np.pi * qmc.LatinHypercube(d=1).random(n=200))
        x_bd = (mesh.func(theta_bd) + eps) * torch.cos(theta_bd)
        y_bd = (mesh.func(theta_bd) + eps) * torch.sin(theta_bd)
        x_if, y_if = mesh.interface_points(500)
        z_inner = torch.vstack((torch.ones_like(radius_o), -torch.ones_like(radius_i)))
        z_bd = torch.ones_like(x_bd)
        print(f'x_inner = {x_inner.size()}')
        print(f'x_bd = {x_bd.size()}')
        print(f'x_if = {x_if.size()}')
        ...
        ax.scatter(x_inner, y_inner, c=z_inner)
        ax.scatter(x_bd, y_bd, c='r')
        sca = ax.scatter(x_if, y_if, c=k_if)
        ax.axis('square')
    ...
    fig, ax = plt.subplots()
    ax.plot(plot_theta, pred_normal, "k-", linewidth=2)
    ax.set_title("Normal Prediction")
